/** Keep in sync with src/lib/categorize.ts */
...
export function categorizeTransaction(input: {
  ...
  categoriesByNameKey: Record<string, string>;
}): string {
  const { merchant, providerCategoryPrimary, merchantRuleCategoryId, categoriesByNameKey } = input;
  if (merchantRuleCategoryId) return merchantRuleCategoryId;
  ...
  if (providerCategoryPrimary) {
    ...
    if (mapped && categoriesByNameKey[mapped]) return categoriesByNameKey[mapped];
  }
  return categoriesByNameKey.other ?? Object.values(categoriesByNameKey)[0] ?? '';
}
